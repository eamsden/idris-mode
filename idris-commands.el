...
(defun idris-refine-metavar ()
  ...
  (let* ((thing (car (idris-thing-at-point)))
         (result (car (idris-eval `(:compatible-identifiers ,thing))))
         (selected (substring-no-properties (popup-menu* result :isearch 't)))
         (newexpr (car (idris-eval `(:make-refined-expression ,selected)))))
    (beginning-of-line)
...
(defun idris-refine-metavar-complete ()
  ...
  (idris-load-file-sync)
  (let* ((thing (car (idris-thing-at-point)))
         (result (car (idris-eval `(:complete-compatible-identifiers ,thing))))
         (selected (substring-no-properties (popup-menu* result :isearch 't)))
...
(defun idris-refine-metavar-recursive ()
  ...
  (let* ((thing (car (idris-thing-at-point)))
         (result (car (idris-eval `(:compatible-identifiers-recursive ,thing)))))
